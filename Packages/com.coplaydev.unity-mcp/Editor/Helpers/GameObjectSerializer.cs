...
namespace MCPForUnity.Editor.Helpers
{
    ...
    {
        private static bool ShouldSkipSerializedProperty(Type componentType, PropertyInfo propInfo)
        {
            if (componentType == null || propInfo == null)
                return false;

            // Unity 6 warns on reflective access to this obsolete AudioSource property.
            if (typeof(AudioSource).IsAssignableFrom(componentType) && propInfo.Name == "rolloffFactor")
                return true;
            ...
            if (!_metadataCache.TryGetValue(cacheKey, out CachedMetadata cachedData))
            {
                ...
                while (currentType != null && currentType != typeof(MonoBehaviour) && currentType != typeof(object))
                {
                    ...
                    foreach (var propInfo in currentType.GetProperties(propFlags))
                    {
                        ...
                        if (ShouldSkipSerializedProperty(componentType, propInfo)) continue;
